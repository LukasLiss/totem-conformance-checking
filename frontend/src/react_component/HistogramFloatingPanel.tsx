...
const TEMPORAL_SHORT_LABELS: Record<string, string> = {
  D: 'D',
  Di: 'D-i',
  I: 'I',
  Ii: 'I-i',
...
export function HistogramFloatingPanel({
  ...
  const initialPos = computeInitialPosition(position, panelWidth, panelHeight);
  const { pos, isDragging, onMouseDown } = useDraggable(initialPos);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                {relationKeys.map((key) => (
                  <th key={key} className="text-right py-1.5 font-medium text-gray-600 px-1 text-xs">
                    {type === 'temporal' ? (TEMPORAL_SHORT_LABELS[key] || key) : (labels[key]?.substring(0, 3) || key)}
                  </th>
                ))}
              </tr>
            </thead>
