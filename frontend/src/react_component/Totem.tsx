...
function TotemInner({
  ...
  const addOrReplacePanel = useCallback(
    (element: SelectedElement) => {
      setSelectedElements((prev) => {
        const existing = prev.findIndex((e) => e.id === element.id);
        if (existing >= 0) {
          // Move to end (top of z-order) and update position
  ...
  const handleEllipseClick = useCallback(
    (source: string, target: string, position: { x: number; y: number }) => {
      ...
      addOrReplacePanel({ id: `ellipse-${source}-${target}`, type: 'ellipse', sourceType: source, targetType: target, position });
    },
    [conformanceData, addOrReplacePanel]
  );

  const handleArcClick = useCallback(
    (source: string, target: string, position: { x: number; y: number }) => {
      if (!conformanceData) return;
      addOrReplacePanel({ id: `arc-${source}-${target}`, type: 'arc', sourceType: source, targetType: target, position });
    },
    [conformanceData, addOrReplacePanel]
  );
  ...
  const handleLogCardClick = useCallback(
    (source: string, target: string, _side: 'source' | 'target', position: { x: number; y: number }) => {
      if (!conformanceData) return;
      addOrReplacePanel({ id: `logCard-${source}-${target}`, type: 'logCard', sourceType: source, targetType: target, position });
    },
    [conformanceData, addOrReplacePanel]
  );
  ...
  // Close a specific floating panel by id
  const closeFloatingPanel = useCallback((id: string) => {
    setSelectedElements((prev) => prev.filter((e) => e.id !== id));
  }, []);

  // Render floating panels based on selected elements
  const renderFloatingPanels = () => {
    if (selectedElements.length === 0 || !conformanceData) return null;

    return selectedElements.map((element) => {
      const { id, type, sourceType, targetType, position } = element;
      const onClose = () => closeFloatingPanel(id);
